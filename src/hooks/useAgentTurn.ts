    ...
    try {
      ...
      const sys = {
        ...
        content: [
          ...
          plan
            ? `PLAN MODE (read-only turn): investigate with read-only tools only (fs_list/read/search/glob, skill_read, git_status/diff/log, nexa_read, browser_snapshot/screenshot, lsp_diagnostics). All writes, shell, browser actions, commits and MCP tools are DISABLED and error if called - do not attempt them, do not narrate them. End with a short numbered plan (files to touch, commands to run, risks). Ask the user to switch to Build to execute.`
            : `Tools: fs_list, fs_read, fs_search (grep content), fs_glob (find files by name), fs_create (empty file/dir), lsp_diagnostics (typecheck one file: ts/rs/py) - read-only-ish, auto-approved. fs_write STAGES to the Diff review gate (user must Approve - never writes directly). fs_rename/fs_delete REQUIRE approval (destructive). Git: git_status/git_diff/git_log are read-only auto-approved; git_commit stages ONLY the listed files and REQUIRES approval - prefer small commits with clear messages. shell_run, browser_navigate/click/type/back REQUIRE explicit user approval via popup - explain what you want before calling. browser_snapshot/screenshot/scroll are auto-approved. browser_screenshot gives you VISION - you SEE the attached page image, so use it to verify layout, styling, and errors visually.`,
