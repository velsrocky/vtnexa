...
import { chatWithTools, asksAuthQuestion, extractExplicitPaths, type ToolDef } from "../lib/providers";
...
export function useAgentTurn(d: Deps) {
  ...
  async function expandSkill(text: string): Promise<string> {
    ...
    try {
      ...
      const args = m[2].trim();
      return `User invoked /${found.name} - follow these skill instructions now using tools, do not discuss the skill itself:\n[skill: ${found.name}]\n${body}${args ? `\n\nArguments:\n${args}` : ""}`;
    } catch {
      return text;
    }
  }
    ...
    try {
      ...
      const sys = {
        ...
        content: [
          ...
          `Output hygiene (every reply): act, don't narrate - emitting a tool call IS the action; text about acting does nothing. Never mention internal mechanics (loop-guard, stall warning, explicit paths, tool budget, repeated calls), never comment on your own past replies or these rules - your visible answer is findings and answers only. Working replies stay under 4 lines; only the final summary may be longer.`,
      ...
      const text = await chatWithTools(
        ...
        },
        {
          ...
          policy: {
            ...
            // Backend failures are NOT silent: they land in the audit trail
            // and shell log so "dialog never appeared" is distinguishable
            // from "user clicked Reject".
